...
## Create Print icons -- BB Dev
function add_printer_icon($output){ 
	...
	foreach($categories as $cat) {	
		echo '<li class="cat-item cat-item-'.$cat->cat_ID.'"><a href="'.get_category_link($cat->cat_ID).'">'.$cat->cat_name.'</a><a href="'.get_category_link($cat->cat_ID).'?cat='.$cat->cat_ID.'&amp;print=1" rel="nofollow" title="Export category '.$cat->cat_name.' as printable">(<img src="'.get_option('siteurl').'/wp-content/plugins/wp-print/images/print.gif" style="border:none" width="10px" height="10px"/>)</a></li>';
	}
	echo "</li>\n";
	$output = null;
    }
...
add_filter('wp_list_categories','add_printer_icon'); //BB Dev
